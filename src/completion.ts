...
class InlineCompletionProvider implements vscode.InlineCompletionItemProvider {
  ...
  private async doGenerateCompletions(
    ...
    if (provider.encode && provider.decode) {
      ...
      prefix = prefixTokens.length ? await provider.decode(prefixTokens) : "";
      suffix = suffixTokens.length ? await provider.decode(suffixTokens) : "";
    } else {
      prefix = header + "\n".repeat(2) + prefixText.trimStart();
